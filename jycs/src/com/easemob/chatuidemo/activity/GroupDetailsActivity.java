...
public class GroupDetailsActivity extends BaseActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!application.isNetworkConnected()) {
            AbToastUtil.showToast(instance, R.string.network_not_connected);
        }
        abHttpUtil = AbHttpUtil.getInstance(GroupDetailsActivity.this);
        ...
        Log.i(TAG, "群组名称:" + group.getGroupName());
        String tt = group.getGroupName();
        ((TextView) findViewById(R.id.group_name)).setText(tt
                + "("
...
//        updateGroup();
    ...
    private void initMemberInfo() {
        String params_member = "";
        List<String> list_member = group.getMembers();
        Log.i(TAG, "群组成员个数:" + list_member.size());
        if (list_member != null && list_member.size() > 0) {
            for (int i = 0; i < list_member.size(); i++) {
                String name = list_member.get(i);
                params_member += name + ",";
            }
        }
        if ("".equals(params_member)){
            AbToastUtil.showToast(application, "没有信息");
            return;
        }
        Log.i(TAG, "请求的参数:" + params_member+"个数:"+list_member.size());
        AbRequestParams params = new AbRequestParams();
        params.put("imUsernames", params_member);
                ...
                new AbStringHttpResponseListener() {
                    ...
                    public void onSuccess(int statusCode, String content) {
                        ...
                        if (ret != null) {
                            ...
                            List<LocalIM> list_im = new ArrayList<LocalIM>();
                            LocalIM im;
                                ...
                                for (User2 user2 : list) {
                                    ...
                                    if (user2 != null) {
                                        ...
                                        if (avator != null) {
                                            ...
                                            im.setUrl(Url.url_base_img_news
                                                    + url);
                                        }
                                        IM im2 = user2.getIm();
                                        if (im2 != null) {
                            ...
                            adapter = new GridAdapter(
                                    ...
                                    group.getMembers(),list_im);
                            Log.i(TAG, "成员个数"+list_im.size());
                            Log.i(TAG, "群成员第一个成员信息" + group.getMembers().get(0));
    ...
    private class GridAdapter extends ArrayAdapter<String> {
        ...
        private List<LocalIM> list;
        public GridAdapter(Context context, int textViewResourceId,
                List<String> objects,List<LocalIM> list) {
        ...
        public View getView(final int position, View convertView,
            ...
            if (position == getCount() - 1) {
            ...
            } else if (position < getCount() -2){ // 普通item，显示群组成员
                final String username = getItem(position);
                Log.i(TAG, "当前的下标:"+position);
                LocalIM im = list.get(position);
                convertView.setVisibility(View.VISIBLE);
               ...
               application.getLoader().display(holder.imageView, url);
